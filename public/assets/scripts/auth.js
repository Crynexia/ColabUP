...
const ColabAuth = (() => {
  ...
  function logout() {
    ...
    // Limpieza defensiva de estado derivado/UX (evita que la UI siga "logueada")
    try {
      // Prefijos usados en el proyecto
      Object.keys(localStorage)
        .filter(k => k.startsWith('colabup_') || k.startsWith('colabup_') || k.startsWith('colabup-'))
        .forEach(k => localStorage.removeItem(k));
    } catch {}

    try {
      // sessionStorage solo guarda la sesión
      Object.keys(sessionStorage)
        .filter(k => k.startsWith('colabup_') || k === SESSION_KEY)
